...
class Common_Word_Forms_Check:
    ...
    def get_base_form_of_word(self, word):
        ...
        orig_word = word
        while True:
            poss_base_words = self.__get_poss_merr_web_tokens__(word)
            poss_base_words += self.__get_poss_word_form_tokens__(word)
            # print('\t', poss_base_words)
            if poss_base_words:
                best_word = self.__closest_poss_base_word__(
        ...
        self.base_form = best_word

        self.cwf_mem[orig_word] = self.base_form
    ...
    def __get_poss_merr_web_tokens__(self, word):
        ...
        s3 = soup.find('p', class_="et")

        poss_base_words = []

        if s0:
            poss_base_words.append(s0.get_text())
        ...
        if s3:
            s3 = s3.get_text()
            s3 = nltk.wordpunct_tokenize(s3)
            poss_base_words += [t.lower() for t in s3]
    ...
    def __get_poss_word_form_tokens__(self, word):
        aD = get_word_forms(word)
        poss_base_words = []
        for k in aD.keys():
            poss_base_words += list(aD[k])

        poss_base_words = self.__purge_poss_base_words__(word, poss_base_words)

        return poss_base_words

    def __get_possible_wiki_tokens__(self, word):
        ...
        poss_base_words = self.__purge_poss_base_words__(
            word, poss_base_words)
...
w_list = [
    ...
    'electricals', 'electrical', 'electricity', 'electrician',
    'fda', 'ul', 'usa',
    'processability', 'processing', "processed",
    'resistance', 'resistant', 'resistivity',
    'thermally', 'thermo', 'thermodynamics', 'thermodynamic',
    'thermodynamically',
    'flammability', 'flame', 'flammable',
    'gubber',
    'recyclable', 'recycle', 'recycling',
    'compostable', 'composting', 'compost'
    'consider', 'considering', 'considerate',
    'containment', "contained", 'container',
    "softening", 'softener', 'soften', 'soft',
    "divided", "light", "park",
    "reduce", 'reduceable', 'reducer', 'reduction',
    "human", 'humanity', 'humanly', 'humans',
    "entrust", 'entrustable', 'entrusting', 'entrusted'
    ]

# w_list = ["flammability", 'flame', 'flammable']
